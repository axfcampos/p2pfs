package pt.tecnico.ulisboa.p2pfs;
	...
	public final class LocalFsManager extends FuseFilesystemAdapterAssumeImplemented
	{
...
//				System.exit(1);
//			}
//			new LocalFsManager().log(true).mount(args[0]);
//		}
		public void mounts() throws FuseException{
			
			this.log(true).mount("testfs");
		}

		private final MemoryDirectory rootDirectory = new MemoryDirectory("");

		public LocalFsManager() throws UnsupportedEncodingException
		{
			// Sprinkle some files around
			MemoryFile mf = new MemoryFile("Sample file.txt", "Hello there, feel free to look around.\n");
			rootDirectory.add(mf);
			String text = "overwritten";
			ByteBuffer contentss = ByteBuffer.allocate(0);
			final byte[] contentBytes = text.getBytes("UTF-8");
			contentss = ByteBuffer.wrap(contentBytes);
			
			mf.write(contentss, text.length(), 0);
